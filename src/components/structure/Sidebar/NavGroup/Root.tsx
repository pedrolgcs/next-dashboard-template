...
function Root({ title, icon: Icon, children }: RootProps) {
  return (
    <Accordion.Item
      value={title}
      className={cn(
        'group flex flex-col rounded',
        'data-[state=open]:border data-[state=open]:border-zinc-200',
      ...
      <Accordion.Header>
        ...
        >
          {Icon && (
            <Icon
              className={cn(
                'h-5 w-5 text-zinc-700 transition group-hover:text-violet-500',
                'dark:text-zinc-300 dark:group-hover:text-violet-300',
              )}
            />
          )}
          ...
          <IconChevronDown
            className={cn(
              'ml-auto h-5 w-5 text-zinc-700 transition group-hover:text-violet-500',
